...
public abstract class EscPosPrinterSize {
    ...
    protected EscPosPrinterSize(
            int printerDpi, float printerWidthMM, int printerNbrCharactersPerLine) {
    ...
    public byte[] bitmapToBytes(Bitmap bitmap) {
        ...
        if (bitmapWidth > maxWidth) {
            bitmapHeight =
                    Math.round(((float) bitmapHeight) * ((float) maxWidth) / ((float) bitmapWidth));
        ...
        if (bitmapHeight > maxHeight) {
            bitmapWidth =
                    Math.round(
                            ((float) bitmapWidth) * ((float) maxHeight) / ((float) bitmapHeight));
