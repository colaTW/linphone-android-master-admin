...
public class PrinterTextParserImg implements IPrinterTextParserElement {
     ...
     * @return A hexadecimal string of the image data. Empty string if Drawable cannot be cast to
     *     BitmapDrawable.
     */
    public static String bitmapToHexadecimalString(
            EscPosPrinterSize printerSize, Drawable drawable) {
        if (drawable instanceof BitmapDrawable) {
            return PrinterTextParserImg.bitmapToHexadecimalString(
                    printerSize, (BitmapDrawable) drawable);
        }
        return "";
    }
     ...
     */
    public static String bitmapToHexadecimalString(
            EscPosPrinterSize printerSize, BitmapDrawable bitmapDrawable) {
        return PrinterTextParserImg.bitmapToHexadecimalString(
                printerSize, bitmapDrawable.getBitmap());
    }
    ...
    public PrinterTextParserImg(
        ...
        this(
                ...
                textAlign,
                PrinterTextParserImg.hexadecimalStringToBytes(hexadecimalString));
    }
     ...
     */
    public PrinterTextParserImg(
            PrinterTextParserColumn printerTextParserColumn, String textAlign, byte[] image) {
        ...
        int byteWidth = ((int) image[4] & 0xFF) + ((int) image[5] & 0xFF) * 256,
                ...
                nbrByteDiff =
                        (int) Math.floor(((float) (printer.getPrinterWidthPx() - width)) / 8f),
        ...
        this.length =
                (int)
                        Math.ceil(
                                ((float) byteWidth * 8)
                                        / ((float) printer.getPrinterCharSizeWidthPx()));
        this.image = image;
    }
