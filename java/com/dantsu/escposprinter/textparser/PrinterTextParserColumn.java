...
public class PrinterTextParserColumn {
    ...
    public PrinterTextParserColumn(PrinterTextParserLine textParserLine, String textColumn)
            throws EscPosParserException, EscPosBarcodeException, EscPosEncodingException {
        ...
        if (!isImgOrBarcodeLine) {
            ...
            while (true) {
                ...
                closeTagIndex++;
                PrinterTextParserTag textParserTag =
                        new PrinterTextParserTag(textColumn.substring(openTagIndex, closeTagIndex));
                ...
                if (PrinterTextParser.isTagTextFormat(textParserTag.getTagName())) {
                    if (textParserTag.isCloseTag()) {
                        ...
                        switch (textParserTag.getTagName()) {
                            ...
                            case PrinterTextParser.TAGS_FORMAT_TEXT_UNDERLINE:
                                if (textParserTag.hasAttribute(
                                    ...
                                    switch (textParserTag.getAttribute(
                                            PrinterTextParser.ATTR_FORMAT_TEXT_UNDERLINE_TYPE)) {
                                        case PrinterTextParser
                                                .ATTR_FORMAT_TEXT_UNDERLINE_TYPE_NORMAL:
                                            textParser.addTextUnderline(
                                                    EscPosPrinterCommands.TEXT_UNDERLINE_LARGE);
                                            textParser.addTextDoubleStrike(
                                                    textParser.getLastTextDoubleStrike());
                                            break;
                                        case PrinterTextParser
                                                .ATTR_FORMAT_TEXT_UNDERLINE_TYPE_DOUBLE:
                                            textParser.addTextUnderline(
                                                    textParser.getLastTextUnderline());
                                            textParser.addTextDoubleStrike(
                                                    EscPosPrinterCommands.TEXT_DOUBLE_STRIKE_ON);
                                            break;
                                    }
                                } else {
                                    textParser.addTextUnderline(
                            ...
                            case PrinterTextParser.TAGS_FORMAT_TEXT_FONT:
                                if (textParserTag.hasAttribute(
                                        PrinterTextParser.ATTR_FORMAT_TEXT_FONT_SIZE)) {
                                    switch (textParserTag.getAttribute(
                                            PrinterTextParser.ATTR_FORMAT_TEXT_FONT_SIZE)) {
            ...
            // =================================================================================================
            // nbrCharForgetted and nbrCharColumnExceeded is use to define number of spaces for the
            // next columns
    ...
    private PrinterTextParserColumn prependString(
            String text,
            byte[] textSize,
            byte[] textColor,
            byte[] textReverseColor,
            byte[] textBold,
            byte[] textUnderline,
            byte[] textDoubleStrike) {
        return this.prependElement(
                new PrinterTextParserString(
                        this,
                        text,
                        textSize,
                        textColor,
    ...
    private PrinterTextParserColumn appendString(
            String text,
            byte[] textSize,
            byte[] textColor,
            byte[] textReverseColor,
    ...
    private PrinterTextParserColumn prependBarcode(
            String textAlign, Hashtable<String, String> barcodeAttributes, String code)
            throws EscPosParserException, EscPosBarcodeException {
        return this.prependElement(
                new PrinterTextParserBarcode(this, textAlign, barcodeAttributes, code));
    }

    private PrinterTextParserColumn appendBarcode(
            String textAlign, Hashtable<String, String> barcodeAttributes, String code)
            throws EscPosParserException, EscPosBarcodeException {
        return this.appendElement(
                new PrinterTextParserBarcode(this, textAlign, barcodeAttributes, code));
    ...
    private PrinterTextParserColumn prependQRCode(
            String textAlign, Hashtable<String, String> qrCodeAttributes, String data)
            throws EscPosParserException, EscPosBarcodeException {
        return this.prependElement(
                new PrinterTextParserBarcode(this, textAlign, qrCodeAttributes, data));
    }

    private PrinterTextParserColumn appendQRCode(
            String textAlign, Hashtable<String, String> qrCodeAttributes, String data)
            throws EscPosParserException, EscPosBarcodeException {
        return this.appendElement(
                new PrinterTextParserQRCode(this, textAlign, qrCodeAttributes, data));
    }

    private PrinterTextParserColumn prependElement(IPrinterTextParserElement element) {
        IPrinterTextParserElement[] elementsTmp =
                new IPrinterTextParserElement[this.elements.length + 1];
